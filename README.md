...
1. **図は説明用のイラストです（実画面のスクリーンショットは未入手）。** 指示書が参照している `input/manual_browser_ver.pdf`（CoeFont社「ブラウザ版 操作マニュアル」）はこのリポジトリに未配置です。同PDFにゲスト画面の画面コピーがある場合は `input/` に置いてご指示ください。 現状は操作位置と画面構成を示す図を収録しています。図には「ภาพประกอบ（説明図）」と明示し、本文にも「実画面と異なる場合がある」旨のタイ語注記を入れてあります。実画面が撮れ次第、差し替えてください。
2. **ボタン名・ラベルは未確定です。** 画面に写っていない表示名は創作していません（図中は `Passcode` などの一般的な語のみ）。実画面の表示に合わせて本文を直してください。
3. **対訳の確認用に** `output/thai_text_for_review.txt`（タイ語・英語・日本語の対訳一覧）を出力しています。文面を直す場合はここと `src/guide.html` の両方ではなく、`src/guide.html` だけを直して再ビルドしてください。
